...
public:
    ...
    void realizeSchema(const Schema& schema, const DatabaseManager& obj_mgr)
    {
    ...
    void getTableNames(std::unordered_set<std::string>& table_names)
    {
        ...
        evalSelect(
            ...
            +[](void* callback_obj, int argc, char** argv, char** col_names) {
                return static_cast<TableNames*>(callback_obj)->addTableName(argc, argv, col_names);
            },
            &select_callback_obj);
    }
    ...
    void evalSelect(const std::string& command, int (*callback)(void*, int, char**, char**), void* callback_obj) const
    {
...
private:
    ...
    std::string openDbFile_(const std::string& db_dir, const std::string& db_file, const bool create_file)
    {
        ...
        if (db_full_filename_.empty()) {
            if (create_file) {
                ...
                throw DBException("Could not find database file: '") << db_dir << "/" << db_file;
            }
        }

        const int db_open_flags = SQLITE_OPEN_CREATE | SQLITE_OPEN_READWRITE;
        sqlite3* sqlite_conn = nullptr;
        auto err_code = sqlite3_open_v2(db_full_filename_.c_str(), &sqlite_conn, db_open_flags, 0);
    ...
    //! to friends only.
    void prepareStatement_(const std::string& command, void** statement) const
    {
    ...
    {
        char* err = 0;
        const int res = sqlite3_exec(db_conn, command.c_str(), callback, callback_obj, &err);
        if (res != SQLITE_OK) {
            switch (res) {
            case SQLITE_BUSY:
    ...
    bool validateConnectionIsSQLite_(sqlite3* db_conn) const
    {
        ...
        FindHelper finder;
        const std::string command = "SELECT name FROM sqlite_master WHERE type='table'";
